...
--
-- Hôte : 127.0.0.1:3306
-- Généré le : dim. 03 mai 2026 à 18:57
...
('00017', 1, '2026-03-26', '', '00003'),
('00017', 2, '2026-03-26', '', '00004'),
('00017', 3, '2026-03-26', '', '00001'),
('00017', 4, '2026-03-26', '', '00001'),
('00017', 5, '2026-03-26', '', '00001'),
('00017', 6, '2026-03-26', '', '00001'),
('00017', 7, '2026-03-26', '', '00001'),
('00017', 8, '2026-03-26', '', '00001'),
('00017', 9, '2026-03-26', '', '00001'),
('00017', 10, '2026-03-26', '', '00001'),
('00017', 11, '2026-03-26', '', '00001'),
('00017', 12, '2026-03-26', '', '00001'),
('00017', 13, '2026-03-26', '', '00001'),
('00017', 14, '2026-03-26', '', '00001'),
('00017', 15, '2026-03-26', '', '00001'),
('10001', 10001, '2026-03-29', '', '00001'),
...
CREATE TABLE IF NOT EXISTS `suivi` (
  ...
  `idCommandeDocument` varchar(5) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `idCommandeDocument` (`idCommandeDocument`)
...
INSERT INTO `suivi` (`id`, `etat`, `dateSuivi`, `idCommandeDocument`) VALUES
(17, 'livrée', '2026-03-26', '2'),
(19, 'en cours', '2026-04-29', '5'),
(20, 'test', '2026-11-11', '6'),
(21, 'en cours', '2026-04-29', '7'),
(22, 'en cours', '2026-04-29', '8'),
(23, 'en cours', '2026-05-03', '10');

--
-- Déclencheurs `suivi`
--
DROP TRIGGER IF EXISTS `ajout_exemplaire`;
DELIMITER $$
CREATE TRIGGER `ajout_exemplaire` AFTER UPDATE ON `suivi` FOR EACH ROW BEGIN
	DECLARE nb INT;
    DECLARE i INT DEFAULT 0;
    DECLARE docId VARCHAR (5);
    DECLARE dateCmd DATE;
    DECLARE maxNum INT DEFAULT 0;
    
    DECLARE idCommande VARCHAR(5);
	IF new.etat = "livrée" AND old.etat <> "livrée" THEN
    	SET idCommande = NEW.idCommandeDocument;
        
        SELECT nbExemplaire
        INTO nb
		FROM commandedocument
        WHERE id = NEW.idCommandeDocument;
        
        SELECT idLivreDvd
        INTO docId
        FROM commandedocument cd
        WHERE cd.id = NEW.idCommandeDocument;
        
        SELECT dateCommande
        INTO dateCmd
        FROM commande c
        WHERE c.id = NEW.idCommandeDocument;
        ...
        WHILE i < nb DO
            ...
            VALUES (docId, maxNum+1+i, dateCmd, '00001');
...
INSERT INTO `utilisateur` (`login`, `password`, `idService`) VALUES
('francoisg', 'unmdp', 1),
('albertD', 'deumdp', 2),
('JackSp', 'troiyen', 3),
('MelissA', 'quatrequatre', 4);
...
--
ALTER TABLE `suivi`
  ADD CONSTRAINT `suivi_ibfk_1` FOREIGN KEY (`idCommandeDocument`) REFERENCES `commandedocument` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT;
COMMIT;
